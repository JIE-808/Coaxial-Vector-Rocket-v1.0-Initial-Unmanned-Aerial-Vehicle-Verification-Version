...
void TVC_Update(float pitch, float roll, float gyro_y, float gyro_x, float gyro_z, float dt)
{
    ...
    left_us = ClampFloat(left_us, MIN(SERVO_LEFT_UP_US, SERVO_LEFT_DOWN_US), MAX(SERVO_LEFT_UP_US, SERVO_LEFT_DOWN_US));
    right_us = ClampFloat(right_us, MIN(SERVO_RIGHT_DOWN_US, SERVO_RIGHT_UP_US), MAX(SERVO_RIGHT_DOWN_US, SERVO_RIGHT_UP_US));
